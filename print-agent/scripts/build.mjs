...
cpSync(join(root, "scripts", "install.bat"), join(releaseDir, "install.bat"));
cpSync(
  join(root, "scripts", "uninstall.bat"),
  join(releaseDir, "uninstall.bat"),
);
...
writeFileSync(
  ...
  [
    ...
    "",
    "IMPORTANTE: copie a pasta FoodWpPrint INTEIRA.",
    "Precisa ter estes arquivos juntos:",
    "  - food-wp-print-agent.exe",
    "  - FoodWpPrint.exe",
    "  - FoodWpPrint.xml",
    ...
    "  2) Aceite o aviso de Administrador (UAC)",
    "",
    "Ou no PowerShell (na pasta da pasta):",
    "  .\\install.ps1",
    "  (precisa do ponto e barra: .\\  — so install.ps1 nao funciona)",
    "",
    "Depois no painel Food WP neste mesmo PC:",
    "  Configurações → Impressão → Conectar agente → escolher impressora → Salvar",
    "  (isso configura a URL da API para o agente imprimir SOZINHO, sem painel aberto)",
    "",
    "A tela Pedidos NAO precisa ficar aberta. O serviço Windows cuida da fila.",
    "",
    "Remover: uninstall.bat (como Administrador)",
